...
/*
 * Key differences from Groovy implementation:
 * 
 * 1. Explicit type declarations everywhere
 *    - Must specify ArrayList<Integer>, List<TreeNode>, etc.
 *    - No 'def' keyword available
 * 
 * 2. Verbose collection operations
 *    - Collections.sort() instead of .sort()
 *    - .get(index) instead of [index]
 *    - .add() instead of <<
 * 
 * 3. Traditional loops
 *    - for (int i = 0; ...) or for (Type item : list)
 *    - No closures like .each { }
 * 
 * 4. String operations
 *    - String concatenation with +
 *    - .repeat() for string multiplication (Java 11+)
 *    - No GString interpolation
 * 
 * 5. More boilerplate
 *    - Explicit constructors
 *    - More verbose syntax overall
 *    - Need to manually create ArrayLists with new ArrayList<>()
 * 
 * 6. Sublist handling
 *    - Need to wrap with new ArrayList<>() when using subList()
 *    - Groovy's range syntax [0..<n] becomes subList(0, n)
 * 
 * The algorithm and logic are identical, just Java requires more
 * explicit code compared to Groovy's concise syntax.
 */
